...
export default function IssueDetailScreen({ route, navigation }) {
  ...
  const formatDate = (d) => d ? new Date(d).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'N/A';
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        ...
        {/* Title */}
        <Text style={styles.title}>{issue.title || issue.categories?.category_name || 'Facility Issue'}</Text>
        ...
        {issue.completion_photo_url && (
          <View style={styles.section}>
            ...
            <Image source={{ uri: issue.completion_photo_url }} style={styles.photo} resizeMode="cover" />
          </View>
        )}
        ...
        <View style={styles.infoGrid}>
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoValue}>{issue.categories?.category_name || 'General'}</Text>
          </View>
          <View style={styles.infoCard}>
          ...
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoValue}>
              {issue.locations ? `${issue.locations.building_name}${issue.locations.room_number ? ` · ${issue.locations.room_number}` : ''}` : 'Not specified'}
            </Text>
          </View>
          {issue.worker && (
            <View style={styles.infoCard}>
              <Text style={styles.infoLabel}>Assigned Worker</Text>
              <Text style={styles.infoValue}>{issue.worker.name}</Text>
            </View>
          )}
          {issue.reporter && (
            <View style={styles.infoCard}>
        ...
        {/* Worker Actions */}
        {user.role === 'worker' && issue.status === 'pending' && (
          <Button title="🔧 Mark as In Progress" onPress={handleMarkInProgress} style={styles.actionBtn} />
        )}
        ...
        <View style={styles.section}>
          ...
          {issue.comments?.map(c => (
            <View key={c.id} style={styles.commentCard}>
              <View style={styles.commentHeader}>
                ...
                <Text style={styles.commentRole}>{c.user?.role?.replace('_', ' ')}</Text>
              </View>
          ...
          {/* Add comment (worker & FM) */}
          {(user.role === 'worker' || user.role === 'facility_manager') && (
...
const styles = StyleSheet.create({
  ...
  commentRole: { fontSize: 11, color: COLORS.textLight, textTransform: 'capitalize' },
